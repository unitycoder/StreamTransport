...
namespace Transport {
  ...
  static extern IntPtr memset(IntPtr dest, int c, UIntPtr byteCount);
    ...
    public static int GetFieldOffset(FieldInfo          fi)                       => GetFieldOffset(fi.FieldHandle);
    public static int GetFieldOffset(RuntimeFieldHandle h)                        => Marshal.ReadInt32(h.Value + (4 + IntPtr.Size)) & 0xFFFFFF;
    public static int GetFieldOffset<T>(string          name) where T : unmanaged => GetFieldOffset(typeof(T).GetField(name));
